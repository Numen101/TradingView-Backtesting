...
7. Consulta el informe nativo y la tabla resumen para el límite activo.
...
El input **Límite usado por el Strategy Tester** decide cuál de ellos genera las órdenes y los resultados de la estrategia. Los otros nueve valores quedan disponibles para cambiar rápidamente la configuración, pero no se simulan en paralelo.
...
La tabla de la esquina inferior derecha del panel muestra seis resultados calculados por la estrategia: equity final, rentabilidad total, máximo drawdown cierre-a-cierre, duración máxima del drawdown, Sharpe anualizado y número de compras. Estas métricas no se duplican en la Ventana de datos.

El drawdown personalizado recorre la equity al cierre de cada sesión. Cada nuevo máximo de equity inicia una referencia y se conserva la mayor caída porcentual posterior hasta un cierre, junto con la mayor duración en días naturales redondeados entre un máximo y su recuperación o el final de la evaluación. Ambas cifras emplean así la misma definición cierre-a-cierre y no dependen de las estadísticas calculadas por TradingView.

El Sharpe personalizado usa los rendimientos simples diarios de la equity durante todo el período configurado, incluidas las sesiones sin posición. Convierte la tasa libre de riesgo anual en su equivalente diario compuesto, calcula la desviación estándar muestral y anualiza el cociente mediante la raíz de 252 sesiones.

El informe nativo y la tabla resumen corresponden al único límite seleccionado y valoran la posición en la última vela completa disponible en el gráfico.
...
- El escenario seleccionado puede realizar múltiples compras, pero solo genera una venta voluntaria en la última vela completa.
